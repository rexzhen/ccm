...
class SessionManager {
  constructor(workingDir = null) {
    // Allow override of working directory (important when run from plugin directory)
  ...
  saveSession(sessionData = {}) {
    ...
    // Generate summary markdown and also store it in the data object
    const summaryMarkdown = this.generateSummary(data);
    data.summary = summaryMarkdown;
  ...
  saveSummary(sessionData) {
    // Use the summary from the data object if it was already generated
    const summary = sessionData.summary || this.generateSummary(sessionData);
    const summaryPath = path.join(this.sessionDir, 'summaries/latest.md');
...
---
*Generated by CCM Plugin v1.0.4*
...
if (require.main === module) {
  ...
  // Allow working directory to be passed via environment variable
  // This is set by save-session.sh after extracting from transcript
  const workingDir = process.env.CLAUDE_PROJECT_CWD || null;
